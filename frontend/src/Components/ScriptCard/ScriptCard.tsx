...
const ScriptCard: React.FC<Props> = ({cardType, input}) => {
    ...
    const [currentScript, setCurrentScript] = useState<Script>(emptyTrack);
    const [scriptList, setScriptList] = useState<Track[]>([]);
    ...
    const resetScriptComponent = () => {
        ...
        setSeed(Math.random());
        setNewScriptList(PopulateTrackCards()); // need to debug. not calling checklike when opening/editing unliked track.
    ...
    async function getPopularScripts(numTracks:number) {

        // hit api for 'numTracks' tracks
        var objArray:Script[] = [];

        await sendAPI('get', '/tracks/getPublicPopularScripts')
        .then((res) => {
    ...
    function PopulateTrackCards() {
        ...
        for(let i = 0; i < MAX_ROWS; i++){
            for(let j = 0; j < MAX_COLS; j++) {
                let currentTrack = scriptList[currentTrackCounter++];
    ...
    return (
        <div className='container text-center'>
            ...
            <Modal id='pop-up' show={show} onHide={handleClose} onExit={resetScriptComponent}>
                <TrackModal key={seed} track={currentScript} closeModal={setShow}/>
            </Modal>
        </div>
    )
};
