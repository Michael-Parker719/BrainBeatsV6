...
export class NoteHandler extends AbstractNoteHandler {
    ...
    public originalNoteGeneration = async (inputStream: Array<number>) => {

        if (this.stopFlag) {//Stop recording
            console.log("stopped");
            clearInterval(this.musicClock);
            this.midiGenerator.setStopFlag();
            return;
        ...
        if(this.warmup)//headset error and/or doesn't output EEG readings
        {
        ...
        else//Recording actually starts
        {
            for(var i = 0; i < this.channelTotal; i++)
            {
                if(this.outlierCheck(i, inputStream[i]))//outlier or first value of recording section
                {
    ...
    private outlierCheck(idVal: number, inValue: number): boolean//True -> Send to Abnormal EEG buffer | False -> Send to Normal EEG buffer
    {
        if(this.EEGaverage[idVal] == 0)//first batch for average record
        {
            return true;
        }
        else if(true)//Outlier check
        {
            if(this.EEGaverage[idVal] * 1.5 < inValue)//Higher Outlier
            {
            ...
            else if(this.EEGaverage[idVal] * 0.5 > inValue)//Lower Outlier
            {
                if(this.AnomlousStreak[idVal] > 0)
                    {
    ...
    {
        if(this.AnomlousStreak[idVal] <= (this.AnomlousStreakMax * -1) )//Higher theta/delta activity -> user is getting sleepy
        {
            this.AnomlousStreak[idVal] = 0;
            this.noteSetNumb = this.noteSetNumb - 1;//Set to lower note set
            if(this.noteSetNumb <= -1)//underflow check
            {
        ...
        else if(this.AnomlousStreak[idVal] >= (this.AnomlousStreakMax * 2))//Lower theta/delta activity -> user is not sleepy 
        {
            this.AnomlousStreak[idVal] = 0;
            this.noteSetNumb = this.noteSetNumb - 1;//Set to lower note set
            if(this.noteSetNumb >= 12)//overflow check
            {
    ...
    private generateNextBeat(idVal: number)
    {
        ...
        if (this.pauseCounter[idVal] >= this.nextPause[idVal])//MIDI channel is available
        {
            ...
            if(NoiseTotal < this.NoiseCapacity)//Is able to be generated without exceding noise capacity(white noise prevention)
            {
    ...
    private noteDeclaration(idVal: number)
    {
        ...
        if( (this.EEGbuffer[idVal].length) != 0)//Checks if there is any normal EEG data to use. Also Divide by zero prevention
        {
            if( ( (this.EEGbuffer[idVal].length) / 4) < this.EEGabnormalBuffer[idVal].length)// Not enough outlier data to change results//------[!!!]------//(TEMPORARY)
            {
                ...
                aveAbNorm = Math.floor(this.average(this.EEGabnormalBuffer[idVal]) * (this.EEGabnormalBuffer[idVal].length) );//get sum of abnormal EEG data from average calcuation
                aveNorm = Math.round( (aveAbNorm + aveAbNorm) / ( (this.EEGbuffer[idVal].length) + (this.EEGabnormalBuffer[idVal].length) ) );//Calculate new average based on both arrays
            }
            else//significant amount of outlier data compared to normal data
            {
        ...
        else if(this.EEGabnormalBuffer[idVal].length != 0)//Only abnormal EEG data present or calculating the first average EEG record for this input channel
        {
            if(this.EEGaverage[idVal] == 0)//Calculate first average EEG record for this input channel
            {
                if(this.EEGabnormalBuffer[idVal].length >= this.aveSampleNumb)//Enough samples for an average?
                {
                ...
                else//Not enough samples so silence
                {
                    console.log("not enough samples, defaulting to generating silent note");
                }
            } 
            else//Use abnormal EEG data to generate next note
            {
