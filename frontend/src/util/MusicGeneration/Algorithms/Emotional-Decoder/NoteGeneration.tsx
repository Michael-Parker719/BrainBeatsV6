...
import { EmotionDecoder, testEmotionDecoder } from "./EmotionDecoder";
...
export class NoteHandler {
    ...
    private NoteDeclarationRaw(ampValue: number, idx: number) {
        ...
        for (var i = 0; i < this.numNotes; i++) {
            ...
            if (
                returnedAmpValue >= this.incrementArr[i] &&
                returnedAmpValue <= this.incrementArr[i + 1]
            ) {
                index = i;
                break;
            }
    ...
    public originalNoteGeneration = async (
        ...
        for (var i = 0; i < size; i++) {
            ...
            if (noteAndOctave.note !== -1) {
                noteOctaveString =
                    noteAndOctave.note + (noteAndOctave.octave + floorOctave).toString();
            ...
            currentNoteData = {
                player: {
                    ...
                    amplitude: curChannelData,
                },
                writer: {
                    noteLengthName,
                    note: noteAndOctave.note,
                    octave: noteAndOctave.octave + floorOctave,
                },
            };
